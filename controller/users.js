...
const studyrecord = async (req, res) => {
  try {
    ...
    const { scriptId, scriptType, time, typingCnt, duration, progress, speed} = req.body;
    const date = new Date(+new Date() + 3240 * 10000).toISOString();
    const datestring = new Date(+new Date() + 3240 * 10000).toISOString().replace(/\T.*/,'');
  
    await Studyrecord.create({ id, scriptId, scriptType, time, typingCnt, date, duration, datestring, progress, speed});
    res.json({
      ok: true,
      message: "등록 완료"
    });
